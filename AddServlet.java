...
//		response.getWriter().append("Served at: ").append(request.getContextPath());
		...
		try {
			...
			String sql = "SELECT MAX(sl_no) FROM winter_internship";
...
//			a=rs.getInt(1);
		    ...
		    s.execute("Insert Into customer (cust_number,name_customer) values ("+cust_number+",'"+cust_number+"1');");
		    
		    
			String query = "INSERT INTO winter_internship (sl_no,business_code,cust_number,clear_date, buisness_year,doc_id,posting_date,document_create_date,due_in_date,invoice_currency,document_type,posting_id,total_open_amount,baseline_create_date, cust_payment_terms, invoice_id) values ("+a+1+",?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
